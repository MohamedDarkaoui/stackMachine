...
template<typename T>
std::string MulData<T>::retrieveInfo() const {
    return "MulNumber()";
}

template<typename T>
void MulData<T>::execute(std::stack<data_t<T>>& dataStack) {
    if (dataStack.size() < 2)
        throw std::runtime_error("Not enough elements in the stack to perform multiplication\n");
